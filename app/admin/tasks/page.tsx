...
export default function TasksAdminPage() {
  ...
  return <main className="authenticated-page"><AuthenticatedHeader user={user} /><div className="admin-shell">
    <div className="admin-title"><div><p className="eyebrow">ADMINISTRAÇÃO DA CASA</p><h1>Tarefas</h1><p>Prepare o que poderá ser sorteado em cada pote de {user.residenceName}.</p></div><span>{tasks.filter((task) => task.isActive).length} ATIVAS</span></div>
    <div className="task-admin-grid">
      <form className="pot-form task-form" onSubmit={save}><span className="step-badge">{editingId ? "EDITAR TAREFA" : "NOVA TAREFA"}</span><h2>{editingId ? draft.name : "Cadastre uma tarefa"}</h2>
        {activePots.length === 0 ? <div className="form-alert">Crie ou reative um pote antes de cadastrar tarefas. <a href="/admin/pots">Gerenciar potes</a></div> : <>
          <label>Pote<select name="potId" onChange={(event) => setDraft({ ...draft, potId: event.target.value })} required value={draft.potId}>{activePots.map((pot) => <option key={pot.id} value={pot.id}>{pot.name}</option>)}</select></label>
          <label>Nome<input autoComplete="off" maxLength={100} minLength={2} name="name" onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder="Ex.: Limpar a geladeira" required value={draft.name} /></label>
          <label>Descrição <small>Opcional, até 300 caracteres</small><textarea maxLength={300} name="description" onChange={(event) => setDraft({ ...draft, description: event.target.value })} placeholder="Inclua alguma orientação útil" value={draft.description} /></label>
      ...
      <section className="house-task-list"><div className="list-header task-list-header"><div><h2>Tarefas da casa</h2><span>{visibleTasks.length} NO FILTRO</span></div><label>Filtrar por pote<select onChange={(event) => setFilterPotId(event.target.value)} value={filterPotId}><option value="all">Todos os potes</option>{pots.map((pot) => <option key={pot.id} value={pot.id}>{pot.name}</option>)}</select></label></div>
        {visibleTasks.length === 0 ? <div className="empty-state">Nenhuma tarefa cadastrada neste filtro.</div> : <div className="house-task-cards">{visibleTasks.map((task) => <article className={!task.isActive ? "is-archived" : ""} key={task.id}><div className="task-card-top"><span className="pot-name-chip">{task.potName}</span><span className={`state-chip ${task.isActive ? "completed" : "pending"}`}>{task.isActive ? "Ativa" : "Arquivada"}</span></div><h3>{task.name}</h3><p>{task.description || "Sem orientações adicionais."}</p><div className="task-card-meta"><strong>{kindLabels[task.kind]}</strong>{task.kind === "recurring" && <span>A cada {task.recurrenceDays} dias</span>}</div><div className="pot-actions"><button onClick={() => edit(task)} type="button">Editar</button><button className="danger-action" onClick={() => toggle(task)} type="button">{task.isActive ? "Arquivar" : "Reativar"}</button></div></article>)}</div>}
      </section>
    </div>
  </div></main>;
}
