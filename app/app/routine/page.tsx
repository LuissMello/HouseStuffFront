...
export default function RoutinePage() {
  ...
  function editEvent(entry: CalendarEntry) { setSelected(null); setEditing(entry); setSuccess(""); setError(""); window.setTimeout(() => document.querySelector(".calendar-event-form")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0); }
  ...
  return <main className="authenticated-page"><AuthenticatedHeader user={user} /><div className="app-shell routine-shell">
    ...
    <CalendarBoard cursor={cursor} entries={entries} loading={calendarLoading} mode={mode} onCursorChange={setCursor} onModeChange={setMode} onSelect={(entry) => { setSelected(entry); setSuccess(""); setError(""); }} />
  ...
  </div>{selected && <CalendarEventDetails entry={selected} onClose={() => setSelected(null)} onDelete={deleteEvent} onEdit={editEvent} />}</main>;
}
